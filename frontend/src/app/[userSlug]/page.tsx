...
import { useEffect } from "react"; // Keep useEffect for logging, maybe
...
export default function UserProfilePage() {
  const { user: loggedInUser, loading: authLoading } = useAuth(); // Viewer info
  const { userSlug } = useParams();
  const profileCollectionSlug = "profile"; // Use the special slug

  // SWR key depends on userSlug. Fetch the "profile" collection for this user.
  const swrKey =
    userSlug && typeof userSlug === "string"
      ? `/api/users/${userSlug}/collections/${profileCollectionSlug}/items`
      : null;
  // ^^^ NOTE: The above key assumes a backend route change is needed for public profiles.
  // For now, we'll stick to the authenticated route for testing, acknowledging it shows the wrong profile.
  const tempSwrKey =
    userSlug && typeof userSlug === "string"
      ? `/api/collections/${profileCollectionSlug}/items`
      : null;
  ...
  const {
    ...
    tempSwrKey, // Pass the key (which can be null)
    // The fetcher function is only called if the key is not null
    async (key) => {
      console.log(`[UserProfilePage] SWR Fetcher called with key: ${key}`);
      // Directly call getCollectionItems with the known slug
      // No need to re-check slug validity here as SWR wouldn't call if key was null
      return getCollectionItems(profileCollectionSlug);
    },
    {
      revalidateOnFocus: false,
      // Add error handling config
      shouldRetryOnError: false, // Prevent retries on 404/403 for example
    }
  );
  ...
  // Determine like state (relevant even if not owner)
  const hasLiked = collection?.currentUserHasLiked ?? false;
  const likeCount = collection?.likeCount ?? 0;
  const isOwner = loggedInUser?.username === userSlug;
  ...
  return (
    <div className="min-h-screen p-4">
      ...
      <div className="mb-4 flex justify-between items-start">
        ...
        <div className="flex items-center space-x-2 mt-6">
          <Button
            ...
            size="sm"
            onClick={() => {
              /* TODO: Implement handleLikeToggle */
            }}
          >
            <Heart
      ...
      {/* TODO: Add Edit Note button only if isOwner */}
      {isOwner && (
        <div className="mb-4">
          <Button
            variant="outline"
